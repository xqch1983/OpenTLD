...
namespace tld
{
	...
	//expanded/unfolded by xqc
	void tldOverlapRect_unfold(int *windows, int numWindows, Rect *boundary, float *overlap)
	{
		int bb1[4], *bb2;
		bb1[0] = boundary->x;
		bb1[1] = boundary->y;
		bb1[2] = boundary->width;
		bb1[3] = boundary->height;
		//tldOverlap(windows, numWindows, bb, overlap);
		...
		for (int i = 0; i < numWindows; i++)
		{
			bb2 = windows + TLD_WINDOW_SIZE * i;

			//overlap[i] = tldBBOverlap(bb, &windows[TLD_WINDOW_SIZE * i]);
			//float tldBBOverlap(int *bb1, int *bb2)
			...
			//if (overlap[i] != 0.0)
			//{
			//	printf("the index is %d, overlap[%d]=%f\n", i, i, overlap[i]);
			//	Sleep(1000);
			//}

			//printf("windowIdx =%d ,bb[0]=%d,bb2[1]=%d,bb[2]=%d,bb[3]=%d\n", i, bb2[0], bb2[1], bb2[2], bb2[3]); Sleep(1000);
		}
...
#pragma omp parallel for num_threads(8)
		for (int i = 0; i < numWindows; i++)
		{
			...
			// printf("thread=%d\n",thread) ;
			//float temp;
			//temp = tldBBOverlap(boundary, &windows[TLD_WINDOW_SIZE * i]);
			//overlap[i] = temp;		
			overlap[i] = tldBBOverlap(boundary, &windows[TLD_WINDOW_SIZE * i]);
